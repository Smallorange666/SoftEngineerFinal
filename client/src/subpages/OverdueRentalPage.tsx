...
import { OverdueRentalInfo } from "../types/common";
...
const OverdueRentalPage: React.FC = () => {
  const [data, setData] = useState<OverdueRentalInfo[]>([]);
  const [filteredData, setFilteredData] = useState<OverdueRentalInfo[]>([]);
  ...
  const handleSearch = (
    ...
    dataIndex: keyof OverdueRentalInfo
  ) => {
    confirm();
  ...
  const getColumnSearchProps = (
    dataIndex: keyof OverdueRentalInfo
  ): Exclude<TableProps<OverdueRentalInfo>["columns"], undefined>[number] => ({
    filterDropdown: ({
      setSelectedKeys,
      selectedKeys,
      confirm,
  ...
  // 列定义
  const columns: ColumnsType<OverdueRentalInfo> = [
    {
      title: "ID",
      dataIndex: "rental_id",
      width: "5%",
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const rentals = await fetchOverdueRental(); // 调用服务函数
      setData(rentals);
  ...
  const handleTableChange: TableProps<OverdueRentalInfo>["onChange"] = (
    pagination
  ) => {
    setTableParams({
      pagination,
...
export default OverdueRentalPage;
